...
TVM_REGISTER_GLOBAL("runtime.disco.allreduce")
    .set_body_typed([](ObjectRef obj, TVMArgValue arg_op_type) -> TVMRetValue {
      int op_type = -1;
      if (arg_op_type.IsObjectRef<ShapeTuple>()) {
        ShapeTuple op = arg_op_type;
        CHECK_EQ(op.size(), 1) << "ValueError: `op` must be an integer enum class";
        op_type = op[0];
      } else {
        op_type = arg_op_type;
      }
      ...
      CHECK(pf != nullptr) << "ValueError: Cannot find the allreduce function for " << ccl
                           << " via `" << pf_name << "`";
      return (*pf)(obj, op_type);
    });
